...
B1DetectorConstruction::B1DetectorConstruction(G4int gX_Layers, G4int gY_Layers, G4int gZ_Layers, G4double gdetUnit_XY)
    : G4VUserDetectorConstruction(),
      X_Layers(gX_Layers), Y_Layers(gY_Layers), Z_Layers(gZ_Layers), detUnit_XY(gdetUnit_XY * mm)
{
}
...
G4VPhysicalVolume *B1DetectorConstruction::Construct()
{
  ...
  */
  ...
  // size
  detUnit_XY = detUnit_XY * mm; // 探测器单元的XY，Fe、crystal、Plastics也是这个值，在B1example.cc中定义
  G4double Detector_XY = X_Layers * detUnit_XY * mm;
  G4double Fe_Z = 1. * mm;
  G4double Pb_Z = 2. * mm;
  ...
  for (Z_index = 1; Z_index < Z_Layers; Z_index++)
  {                       // 因为第一块Crystal编号0，这里从编号1开始
    copynumber = Z_index; // copynmuber == -1 都是不要的
  ...
  */
  ...
  /* Detector combined with 
    1 × 10 detectorArray with index X
             -→ Y轴
    ▓▓▓▓▓▓▓▓
    ▓▓▓▓▓▓▓▓
    ▓▓▓▓▓▓▓▓
